	...
	namespace util{
		namespace multicast{
			...
			public:
				...
				sender(boost::asio::io_service& _ioService, const boost::asio::ip::address& _multicast, uint16_t _port);
				...
				uint32_t send(Buffer && _buffer, std::size_t _size){
					return udpSocket.send_to(boost::asio::buffer(std::forward<Buffer>(_buffer), _size), endPoint);
				}
